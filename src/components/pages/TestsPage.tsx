...
export function TestsPage() {
  ...
  //     setSelectedTest(testId);
  // setIsTestStarted(true);
  // toast.success(`Starting ${test.subject} test. Good luck!`);
  ...
  // ✅ Map UI subject names to actual collection names in MongoDB
...
// start test function fetches real questions from backend
  const startTest = async (testId: string) => {
  const test = availableTests.find(t => t.id === testId);
  if (!test) return;

  console.log("Clicked test:", test);
  console.log("Mapped subjectKey:", subjectMap[test.subject]);
  ...
  try {
    // ✅ Fetch real test questions from backend
    const subjectKey = test.subject
      .toLowerCase()
      .replace(/\s*&\s*/g, "")
      .replace(/\s+/g, "")
      .replace(/[^\w]/g, ""); 
    // converts names like "Computer Networks" → "cn"

    const questions = await fetchTestQuestions(subjectKey);
    ...
    // ✅ Here you can navigate or display the test UI with fetched questions
    toast.success(`Loaded ${questions.length} questions for ${test.subject}`);
    
    // Example: open a modal or new page with the test
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              const subjectTests = testResults.filter(test => test.subject === subject);
              const avgScore = subjectTests.length > 0 
                ? Math.round(subjectTests.reduce((sum, test) => sum + test.score, 0) / subjectTests.length)
                : 0;
